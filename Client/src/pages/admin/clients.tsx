...
const Clients = () => {
  ...
  const displayedUsers = clients.slice(
    (currentPage - 1) * ITEMS_PER_PAGE,
    currentPage * ITEMS_PER_PAGE
  );
  
  const handleShowAlert = (userId: string) => {
    setClientId(userId)
    setShowAlert(true)
  }
  
  const clientBlockUnblock = async(userId: string) => {    
    setBlockLoading(prev => ({ ...prev, [userId]: true }));
    try {
      const response = await clientBlock(userId as string)
      if(response.success){
  ...
  return (
    <div className="flex h-screen">
      ...
      <AlertDialog open={showAlert} onOpenChange={setShowAlert}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className='text-2xl text-white'>Confirm </AlertDialogTitle>
            <AlertDialogDescription className='text-lg text-white'>
              Are you sure you want to do this action?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className='text-white hover:bg-gray-900'>Cancel</AlertDialogCancel>
            <AlertDialogAction className='text-black bg-slate-300 hover:text-black hover:bg-white' onClick={() => clientBlockUnblock(clientId)}>Confirm</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <div className="flex-1 p-6 bg-[#000000] pl-40 pt-24">
        <h1 className='flex justify-center text-3xl font-semibold text-gray-200'>
          Client Management
        </h1>
